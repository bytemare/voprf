...
func testExport(t *testing.T, client *Client, export *State) {
	...
	if client.preprocessedBLind != nil {
		if !bytes.Equal(export.PreprocessedBlind.BlindedPubKey, client.preprocessedBLind.blindedPubKey.Bytes()) {
			t.Fatal("blinded pubkey is not correct")
		}

		if !bytes.Equal(export.PreprocessedBlind.BlindedGenerator, client.preprocessedBLind.blindedGenerator.Bytes()) {
			t.Fatal("blinded generator is not correct")
		}
	}
}
