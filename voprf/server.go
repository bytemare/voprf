...
func (s *Server) SetKeyPair(privateKey *ecc.Scalar, publicKey *ecc.Element) error {
	if err := checkKeys(s.Group, privateKey, publicKey); err != nil {
		return err
	}
...
func (s *Server) evaluate(
	...
	if s.Mode == internal.VOPRF {
		proofC, proofS = s.GenerateProof(r, s.privateKey, s.publicKey, blinded, evaluated)
	} else { // POPRF
		proofC, proofS = s.GenerateProof(r, s.t, s.tweakedKey, evaluated, blinded)
	}
...
func (s *Server) setKeyPair(privateKey *ecc.Scalar, publicKey *ecc.Element) {
	...
	if s.Mode == internal.POPRF {
		s.scalar, s.t = s.TweakPrivateKey(privateKey)
		s.tweakedKey = s.Group.Base().Multiply(s.t)
	} else {
