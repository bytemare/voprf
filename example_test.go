...
func ExampleBaseMultiplicative() {
	...
	//Evaluation, err := evaluation.encode(enc)
	//if err != nil {
	//	panic(err)
	//}
	...
	//unblinded, err := client.unblind(evaluation, nil, nil)
	//if err != nil {
	//	panic(err)
	//}

	output, err := client.Finalize(evaluation, info)
	if err != nil {
		panic(err)
	}

	// This is not part of the protocol, but allows output verification
	if server.VerifyFinalize(input, output, info) {
...
//	client, err := RistrettoSha512.ClientAdditive(serverPubKey, preprocessed)
//	if err != nil {
//		panic(err)
//	}
//
...
//	// The client wraps up the protocol execution, by decoding the received evaluation,
//	// reverting the blinding and hashing the protocol transcript.
//	output, err := client.Finish(Evaluation, enc)
//	if err != nil {
//		panic(err)
//	}
//
...
func ExampleVerifiableMultiplicative() {
	...
	client, err := RistrettoSha512.Client(pubKey)
	if err != nil {
		panic(err)
	}
	...
	// The client wraps up the protocol execution, by decoding the received evaluation,
	// reverting the blinding and hashing the protocol transcript.
	//unblinded, err := client.unblind(evaluation.element[0].Bytes(), evaluation.proofC.Bytes(), evaluation.proofS.Bytes())
	//if err != nil {
	//	panic(err)
	//}
	//
	//output := client.finalize(unblinded, info)
